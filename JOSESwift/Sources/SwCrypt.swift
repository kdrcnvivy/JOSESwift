...
open class CC {
    ...
    public static func encryptAuth(blockMode: AuthBlockMode, algorithm: Algorithm,
                                 data: Data, aData: Data,
                                 key: Data, iv: Data, tagLength: Int) throws -> (Data, Data) {
        let cryptFun = blockMode == .gcm ? GCM.crypt : CCM.crypt
        return try cryptFun(.encrypt, algorithm, data,
                                             key, iv, aData, tagLength)
            
      
    }
    public static func decryptAuth(blockMode: AuthBlockMode, algorithm: Algorithm,
                                 data: Data, aData: Data,
                                 key: Data, iv: Data, tagLength: Int) throws -> Data {
        let cryptFun = blockMode == .gcm ? GCM.crypt : CCM.crypt
            //let cipher = data.subdata(in: 0..<(data.count - tagLength))
            //let tag = data.subdata(
            //    in: (data.count - tagLength)..<data.count)
            let (plain, _) = try cryptFun(.decrypt, algorithm, data,
                                             key, iv, aData, tagLength)
//            guard tag == vTag else {
//                throw CCError(.decodeError)
//            }
